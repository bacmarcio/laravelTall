        ...
        <div class="flex gap-4">
            ...
            <div class="bg-white w-[350px] p-4 mt-4 h-48">
                <div class="text-sm text-green-700">
                    Welcome to Grupo AVS! FREE Delivery on your first order to UK or Ireland.
                    Select this opyion at checkout.
                </div>
                ...
                <Link
                    ...
                    :disabed="Number(total)===0.00"
                    :class="Number(total)===0.00 ? 'bg-gray-400' : 'bg-yellow-400 hover:bg-yellow-500'" as="button"
                    :href="$page.props.auth.user != null ? route('checkout.index',{
                            total: totalWithoutDot(), total_decimal: total, items: cart }):route('login')"> Proceed to
                Checkout
                <!-- method="post"
                    as="button" :href="$page.props.auth.user != null ? route('checkout.index',{
                            total: totalWithoutDot(), total_decimal: total, items: cart }):route('login')" -->
                </Link>
            </div>
        </div>
    </AuthenticatedLayout>
